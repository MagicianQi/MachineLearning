...
def update_alpha(svm, type):
    ...
    if type[0] == 'All':
        i = type[1]
    if type[0] == 'SupportVector':
        i = select_alpha_i(svm)
    if not KKT_condition_judgment(svm, i):
        error_i = get_error(svm, i)
        j = select_alpha_j(svm, i, error_i)
...
def select_alpha_i(svm):
    """
    选择SMO算法的第一个alpha，即违反KKT条件最严重的点
    :return: 第 i 个
    """
    if len(svm.alphasIndex) != 0:
        gsp = []
        for k in svm.alphasIndex:
            gsp.append(get_gap(svm, k))
        i = svm.alphasIndex[gsp.index(max(gsp))]
        return i
    else:
        i = int(random.uniform(0, svm.numSamples))
        return i


def get_gap(svm, i):
    """
    计算可行间隙大小，即原始问题与对偶问题的解得间隙
    :return: 间隙大小
    """
    func_d = get_func_distance(svm, i)
    kesai = max(0, 1 - func_d)
    gsp = svm.alpha[i] * (func_d - 1 - svm.train_y[i] * svm.b) + svm.C * kesai
    return gsp


def select_alpha_j(svm, i, error_i):
    """
    选择SMO算法的第二个alpha
    :return: 第 j 个alpha
    """
    while 1:
        if len(svm.alphasIndex) not in [0, 1]:
            sv_error = []
            for k in svm.alphasIndex:
                sv_error.append(abs(get_error(svm, k) - error_i))
            j = svm.alphasIndex[sv_error.index(max(sv_error))]
            if i != j:
                return j
        else:
...
def KKT_condition_judgment(svm, k):
    ...
    :return:判断结果
    """
    # 函数距离小于1，那么alpha必须等于C。函数距离大于1，那么alpha必须要等于0。否则就违反了KKT条件。
...
def train(train_x, train_y, C, thresh, kernelOption=('G', 1.0)):
    ...
    while not KKT_condition(svm, 'All'):
        ...
        for i in svm.alphasIndex:
            revise_count += update_alpha(svm, ('All', i))
        ...
        while not KKT_condition(svm, 'SupportVector'):
            ...
            for j in range(len(svm.alphasIndex)):
                revise_count += update_alpha(svm, ('SupportVector', 0))
...
if __name__ == '__main__':
    ...
    svm = train(x[40:100, :], y[40:100, :], 0.6, 0.001, ('rbf', 1))      # polynomial
    print(predict(svm, x[0:40, :], y[0:40, :]))
