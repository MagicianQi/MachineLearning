...
"""
第二个版本：
1.修改学习速率随迭代次数而减小，尽量少的迭代次数得到更高的准确率
"""
...
def train(x, y, alpha, maxIter, alpha_rate):
    """
    功能：训练函数(变为梯度上升)
    参数：训练集，标签，学习速率，最大迭代次数,学习速率下降速度
    返回：参数
    """
    ...
    for i in range(maxIter):
        ...
        print("Iter:" + str(i+1), "    l:" + str(l(x, y, theta).tolist()[0][0]))
    return theta


def train_SGD(x, y, alpha, maxEpoch, alpha_rate):
    """
    功能：训练函数(SGD)
    参数：训练集，标签，学习速率，最大迭代次数，学习速率下降速度
    返回：参数
...
def get_accuracy(x, y, theta):
    ...
    m = x.shape[0]
    num = 0
    predict_y = sigmoid(x * theta)
    difference = np.abs(y - predict_y).tolist()
    for i in range(m):
...
if __name__ == '__main__':
    ...
    # theta = train(train_x, train_y, 0.01, 100, 0.2)
    theta = train_SGD(train_x, train_y, 0.02, 15, 0.02)
    print(theta)
